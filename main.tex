...
% Workaround for tufte-book + TeX Live 2022 case-changing breakage.
% tufte-common's \allcaps and \smallcaps expand to \MakeTextUppercase /
% \MakeTextLowercase (textcase), and even rerouting to the LaTeX
% kernel's \MakeUppercase / \MakeLowercase still breaks under lualatex
% on TL2022.  Drop case-changing entirely from these macros -- the
% small-caps font shape (\scshape) provides the visual effect on its
% own; the typographic difference is a faux-smallcaps fallback for
% characters the font lacks, which we don't need.
...
\par CC BY-NC-SA 4.0\ [\url{http://creativecommons.org/licenses/by-nc-sa/4.0}]
