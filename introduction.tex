...
Traditional probability feels uncomfortable here.  The chances of the ball landing at any given place is essentially zero and so miraculous. Yet anyone watching this process would only occasionally be surprised by the outcome.
...
For symmetric or monotonic one dimensional continuous distributions, the connection between luck and $p$-values can be pretty simple.  However, the relation between $p$ and $L$ can be complex.  Figure~\ref{fig:intro-chi2} shows the $p$-values vs luck for $\rchi^2_k(x)$ for $k=4$ distribution.
...
\section{Exercises}
\begin{enumerate}
  \item \label{q:intro-treasure} When searching for lost treasure, you should prefer to be lucky ($L \approx 1$). Why? Can you think of other scenarios where the natural language notion of luck (now {\it that} was lucky) aligns with the mathematical definition ($L(\text{that}) \approx 1$)?
  \item \label{q:intro-bus} When riding the bus to work, you should prefer to be unlucky ($L \approx 0$). Why? Can you think of other scenarios where the natural and mathematical notions differ?
  \item \label{q:intro-password} For a typical set of possible passwords used by typical users, what kind of passwords would be lucky, and what kind would be unlucky?
  
\item \label{q:intro-coin-max} Take the maximum of two fair coin flips (0 or 1), so $\Prob(\max = 0)=1/4$ and $\Prob(\max = 1)=3/4$.  What is the luck of each outcome?
\item \label{q:intro-coin-max-3} Now have three independent trials of problem \ref{q:intro-coin-max}. What is the luck now?
...
\item \label{q:intro-coin-human-test} Now write down what feels like a random sequence of six zeros and ones --- don't think too hard about it!  Use these in pairs like problem \ref{q:intro-coin-test}.  What was the luck of this outcome?
\item \label{q:intro-exp} Calculate the luck $L(x)$ for the exponential distribution $p(x)=\lambda \exp(-\lambda x)$ for $x\geq 0$ and $\lambda > 0$.

\item \label{q:intro-rgd-1} In {\em Rosencrantz and Guildenstern Are Dead}, Guildenstern tosses 85 heads in a row at the beginning of the play.  Taken as a sequence of fair coin tosses, why is Guildenstern's luck 1/2?

\item \label{q:intro-rgd-2} Now instead look at the probabilities that the number of heads are as sampled (this generally involves the bionomial distribution but the reported outcome is the simplest case).  Show in this case Guildenstern is insanely lucky:
  \begin{equation*}
...
\begin{itemize}
  \item A\ref{q:intro-treasure} The probability of finding a lost treasure is small, and other possibilities are much more likely, adding in all these more likely probabilities together before accounting for the unlikely cases of finding treasure would lead to $L \approx 1$. Other cases where the ``winning'' scenario is also the least likely fit this, such as hitting every green light on a random commute through a city.
  \item A\ref{q:intro-bus} The probability of getting to the desired destination on time and unharmed is the most likely outcome in a well-functioning city. It would be typically unlikely to have a breakdown.  In this value-free definition of luck, having the bus break down would be called lucky ($L \approx 1$) because such an outcome is unlikely, just like finding treasure in the previous example. Other scenarious where ``losing'' is unlikely would be like this, for example to be diagnosed with an unusual disease.
  \item A\ref{q:intro-password} A typical unlucky password would be ``abc123'', and a typical lucky one would be ``713897303aabed4eb501c6b9af18cc''.  Even though the probabiilty of choosing ``abc123'' is small, there are many other choices which have even smaller probabilities. While for the ``713\ldots'' case there are many passwords that are more likely to be used, hence $L \approx 1$.
\item A\ref{q:intro-coin-max} The domain is $\{0,1\}$, the two possible values of the maximum, with $p(1)=\frac{3}{4}$ and $p(0)=\frac{1}{4}$, in decreasing order of probability.  So $L(1)=\frac{1}{2} p(1) = \frac{1}{2} \cdot \frac{3}{4} = \frac{3}{8}$, and $L(0)=p(1)+\frac{1}{2} p(0) = \frac{3}{4} + \frac{1}{2} \cdot \frac{1}{4} = \frac{7}{8}$.
\item A\ref{q:intro-coin-max-3} For a single trial, the probability for a maximum of $m$ is
\begin{equation}
  p_1(m)=\begin{cases}
           3/4& \text{ if } m = 1,\\
           1/4& \text{ if } m = 0
...
\item A\ref{q:intro-coin-test} The luck of the outcome depends on the specific outcome generated. However, on average, the luck of the outcome is $\frac{1}{2}$.

\item A\ref{q:intro-coin-human-test} The luck of the outcome depends on the specific sequence generated. However, the luck is typically not near $\frac{1}{2}$ because humans are typically bad at making uniformly random numbers.

\item A\ref{q:intro-exp} The luck of the exponential distribution is given by:
\begin{align*}
...
\item A\ref{q:intro-rgd-1} Taken as a sequence of fair coin tosses, the probability of getting 85 heads in a row is $(1/2)^{85}$, which is a very small probability. However, the probability of getting any specific sequence of 85 coin tosses is also $(1/2)^{85}$, so the luck of getting this specific sequence is neither lucky nor unlucky.

\item A\ref{q:intro-rgd-2} When looking at number of heads, getting $0$ or $85$ heads out of $85$ trials are the two most unlikely outcomes with $p(0)=p(85)=(\frac{1}{2})^{85}$. Since the probabilities add to $1$, the sum of all the more likely outcomes must be $p(\Omega)=1-2^{-84}$ (just omitting these two cases).  Adding $\frac{1}{2}$ of the sum of these last two cases gives $L(0)=L(85)=1-2^{-84}+2^{-85}=1-2^{-85}\approx 1-2.585\times 10^{-26}$.
