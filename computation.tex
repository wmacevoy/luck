...
\begin{figure}
\caption{\label{fig:numluckprog}Use the above functions for estimating luck numerically ({\tt nlucks}) and estimating the error in luck (numerical standard deviation).  The last lines optionally compute the exact values of luck ({\tt lucks}) and standard deviations to compare with.  One run produced {\tt nluck=0.63025} and {\tt luck=0.62875} with error bound estimates {\tt nsd=0.004827}, {\tt sd=0.0048314} and {\tt z=0.3105}.}
\lstset{language=Scilab}
\begin{lstlisting}
// get sample set
nsamps=10000;
