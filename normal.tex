...
\intertext{and}
\omega(x) =& \left\{ y \middle| |\sqrt{\sigma^{-1}}(y-\mu)| = |\sqrt{\sigma^{-1}}(x-\mu)| \right\} \,.
\end{align}
Since $\omega(x)$ has no volume, $|\omega(x)|=0$.  So
\begin{align}
L(x) &=|\Omega(x)|\\
L(x) &=\int_{\Omega(x)} P_{\text{normal}}(y;\mu,\sigma) \,dy\,.
\end{align}

By writing changing variables to $z=\sqrt{\sigma^{-1}} (y-\mu)$,
\begin{equation}
