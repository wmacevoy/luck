...
\begin{align}
L(x,y) &\approx \frac{1}{2}\left[1+\erf(R_{xy}(x,y)-\bar{R}_{xy}) \right] \,,
\intertext{where}
R_{xy}(x,y) &= \sqrt{R_x(x)^2+R_y(y)^2} \,, \\
\bar{R}_{xy} &= \sqrt{\bar{R}_x^2 + \bar{R}_y^2 + \frac{1}{2}} \,, \\
R_x(x) &= \sqrt{\left(\Sigma^{(x)}\right)^{-1}} \left(x-\mu^{(x)}\right) \,, \\
R_y(x) &= \sqrt{\left(\Sigma^{(y)}\right)^{-1}} \left(y-\mu^{(y)}\right) \,, \\
\bar{R}_x &= \sqrt{n_x - \frac{1}{2}} \,, \\
\bar{R}_y &= \sqrt{n_y - \frac{1}{2}} \,.
\end{align}
